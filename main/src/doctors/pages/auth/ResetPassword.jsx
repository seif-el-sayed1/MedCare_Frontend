...
import { Globe, Stethoscope, Eye, EyeOff, ArrowLeft, ArrowRight, Sun, Moon } from 'lucide-react'
...
const ResetPassword = () => {
  ...
  const onSubmit = async (data) => {
    setLoading(true)
    try {
      const res = await resetPassword(token, { password: data.password, confirmPassword: data.confirmPassword }, lang)
      ...
      >
        ...
        <div className="relative z-10 max-w-md my-auto space-y-6">
          <span className="inline-block bg-white/15 border border-white/10 text-white text-xs font-semibold px-3 py-1 rounded-full backdrop-blur-sm">
            {lang === 'ar' ? 'إعادة تعيين آمنة' : 'Secure Reset'}
          </span>
          <h1 className="text-white font-bold text-[32px] xl:text-[36px] leading-[1.2] tracking-tight">
            {lang === 'ar' ? 'استعادة\nكلمة المرور' : 'Reset Your\nPassword'}
          </h1>
          <p className="text-white/80 text-sm font-medium leading-relaxed">
            {lang === 'ar'
              ? 'أنشئ كلمة مرور جديدة قوية لحماية حسابك الطبي.'
              : 'Create a new strong password to secure your medical account.'}
          </p>
        </div>
        ...
        <div className="w-full max-w-[420px] mx-auto my-auto space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-neutralSurface-200 dark:border-gray-700 shadow-sm p-6 sm:p-8 space-y-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              >
                {loading 
                  ? (lang === 'ar' ? 'جاري التعيين...' : 'Resetting...') 
